...
/** The foot of the paper form: what an evaluator fills in after the sitting. */
...
const BOOLEAN_FIELDS = ['needs_remedial', 'reviewed'];
...
class Store {
  ...
  #applyUpdate(id, fields) {
    const sets = [];
    const args = [];
    for (const key of EVALUATOR_FIELDS) {
      if (!(key in fields)) continue;
      sets.push(`${key} = ?`);
      const value = fields[key];
      args.push(BOOLEAN_FIELDS.includes(key)
  ...
  async updateSubmission(id, fields) {
    this.#applyUpdate(id, fields);
    return this.#submissionRow(id);
  }

  /**
   * Applies the same evaluator details to many submissions at once. Returns
   * how many rows changed, so the caller can report it honestly.
   */
  async updateSubmissions(ids, fields) {
    if (!ids.length) return 0;
    let changed = 0;
    // One transaction: a half-applied batch would leave the records
    // inconsistent with what the admin was told.
    this.db.exec('BEGIN');
    try {
      for (const id of ids) {
        if (this.#applyUpdate(id, fields)) changed += 1;
      }
      this.db.exec('COMMIT');
    } catch (error) {
