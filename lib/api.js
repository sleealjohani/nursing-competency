...
const {
  getStore, passwordFromEnv, usingDefaultPassword, storageHealth,
} = require('./store');
const {
  FORM_INDEX, TOTAL_ITEMS, getForm, formType, translationFor, scoring,
} = require('./forms');
...
/**
 * @param {number} status
 * @param {string} code     machine-readable, so the browser can show the
 *                          message in the reader's language
 * @param {string} message  English fallback
 * @param {object} details  values the translated message interpolates
 */
...
async function readBody(req, limit = 1_000_000) {
  ...
  if (req.body !== undefined && req.body !== null) {
    if (typeof req.body === 'string') {
      try { return JSON.parse(req.body); } catch { throw new HttpError(400, 'bad_request', 'Invalid JSON body'); }
    }
    return req.body;
  }
  const chunks = [];
  ...
  for await (const chunk of req) {
    size += chunk.length;
    if (size > limit) throw new HttpError(413, 'too_large', 'Request too large');
    chunks.push(chunk);
  }
...
/**
 * Submission ids from a comma-separated parameter.
 *
 * Number('') is 0, not NaN, so a missing or trailing comma would otherwise
 * become a plausible-looking id 0 that matches nothing — and an absent
 * parameter would look like a selection of one.
 */
function parseIds(value, max = 20000) {
...
async function handleApi(req, res, url) {
  ...
  const requireAdmin = async () => {
    if (!await isAdmin()) throw new HttpError(401, 'not_signed_in', 'Not signed in');
  };
  ...
  if (method === 'GET' && pathname.startsWith('/api/competencies/')) {
    ...
    if (!form) throw new HttpError(404, 'unknown_competency', 'No such competency');
    return sendJson(res, 200, {
      form,
      // A reading aid only; the form above stays the record.
      translation: translationFor(form.id),
  ...
  if (method === 'POST' && pathname === '/api/nurses') {
    ...
    if (!jobNumber) throw new HttpError(400, 'job_number_required',
  ...
  if (method === 'GET' && pathname.startsWith('/api/nurses/')) {
    ...
    if (!nurse) throw new HttpError(404, 'not_registered_yet', 'Not registered yet');
    return sendJson(res, 200, { nurse });
  }
  ...
  if (method === 'POST' && pathname === '/api/submissions') {
    ...
    if (!form) throw new HttpError(400, 'unknown_competency', 'No such competency');

    const nurse = await store.nurseByJobNumber(str(body.jobNumber, 60));
    if (!nurse) {
      throw new HttpError(400, 'not_registered',
        'Register your details before submitting');
    }
    ...
    if (!result.complete) {
      throw new HttpError(400, 'incomplete',
        `${result.unanswered} item(s) still unanswered`,
  ...
  if (method === 'GET' && /^\/api\/submissions\/\d+$/.test(pathname)) {
    ...
    if (!submission) throw new HttpError(404, 'not_found', 'Not found');
  ...
  if (method === 'GET' && pathname === '/api/admin/session') {
    ...
    return sendJson(res, 200, {
      ...
      // Only told to someone already signed in.
      usingDefaultPassword: signedIn ? await usingDefaultPassword() : false,
    });
  }
  ...
  if (method === 'POST' && pathname === '/api/admin/password') {
    await requireAdmin();
    if (passwordFromEnv()) {
      throw new HttpError(409, 'password_from_env',
  ...
  /** Fill the evaluator's details on many submissions at once. */
  if (method === 'PATCH' && pathname === '/api/admin/submissions') {
    ...
    if (!ids.length) {
      throw new HttpError(400, 'no_selection', 'No submissions selected');
    }
    if (ids.length > 1000) {
      throw new HttpError(400, 'too_many', 'Too many submissions at once');
    }
    const updated = await store.updateSubmissions(ids, body.fields || {});
  ...
  if (method === 'PATCH' && /^\/api\/admin\/submissions\/\d+$/.test(pathname)) {
    ...
    if (!await store.submissionById(id)) {
  ...
  /**
   * Everything the ZIP export needs: the same payload as /print, but grouped
   * by nurse so one PDF can be built per employee.
   */
  if (method === 'GET' && pathname === '/api/admin/export-groups') {
    await requireAdmin();
    const ids = parseIds(q.get('ids'));
    const rows = ids.length
      ? await store.submissionsByIds(ids)
      : (await store.listSubmissions({ ...filtersFrom(q), limit: 1000 })).rows;
    if (!rows.length) {
      throw new HttpError(400, 'no_selection', 'No submissions selected');
    }
    ...
    for (const submission of rows) {
      const form = getForm(submission.form_id);
      if (form) forms[form.id] = form;
      const key = submission.nurse_job_number || submission.nurse_name;
      if (!byNurse.has(key)) {
        byNurse.set(key, {
          name: submission.nurse_name,
          jobNumber: submission.nurse_job_number,
          unit: submission.nurse_unit,
          submissions: [],
        });
      }
      byNurse.get(key).submissions.push(submission);
    }
    // Oldest first inside each nurse's file, so the PDF reads as a history.
    const groups = [...byNurse.values()].map((group) => ({
      ...group,
      submissions: group.submissions
        .sort((a, b) => String(a.exam_date).localeCompare(String(b.exam_date))),
    })).sort((a, b) => a.name.localeCompare(b.name, 'en'));

    return sendJson(res, 200, { groups, forms, passMark: scoring.PASS_MARK });
  }
  ...
  throw new HttpError(404, 'unknown_endpoint', 'Unknown endpoint');
}
...
async function serveApi(req, res, url) {
  try {
    ...
    if (error.isConfigError) {
      ...
      if (!res.headersSent) {
        sendJson(res, 503, { error: error.message, code: 'storage_unavailable' });
      }
      return;
    }
    console.error(error);
